...
export const PageColorPalette = memo(({ colors, onColorSelect }: PageColorPaletteProps) => {
  ...
  if (!colors || colors.length === 0) {
    return (
      <div className="text-center py-4 text-wisp-elements-textTertiary text-xs">
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5 text-[10px] text-wisp-elements-textTertiary uppercase tracking-wide">
        ...
        {uniqueColors.map((color, index) => {
          ...
          return (
            <div key={`${color}-${index}`} className="flex flex-col items-center gap-1">
              ...
              <span className="text-[9px] text-wisp-elements-textTertiary font-mono truncate max-w-[44px]">
      ...
      <div className="pt-2 border-t border-wisp-elements-borderColor">
        <p className="text-[10px] text-wisp-elements-textTertiary text-center">Click to copy • Double-click to apply</p>
      </div>
    </div>
  );
});
